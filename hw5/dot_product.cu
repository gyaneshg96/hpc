...
__global__ void dotproduct_kernel2(double* sum, const double* a, const double*b, long N){
...
int main() {
  ...
  dotproduct_kernel2<<<Nb,BLOCK_SIZE>>>(dot_d, x_d, y_d, N);
  while (Nb > 1) {
    long N = Nb;
    Nb = (Nb+BLOCK_SIZE-1)/(BLOCK_SIZE);
    dotproduct_kernel2<<<Nb,BLOCK_SIZE>>>(dot_d + N, dot_d, N);
    dot_d += N;
  }
